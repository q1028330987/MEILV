...
<div class="page-container">
	...
	<!-- <div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="member_add('添加用户','<?php echo U('editor');?>','','510')" class="btn btn-primary radius">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr class="text-c" >
				...
				<td><?php echo ($vo['sex'] == 2 ?'男':'女'); ?></td>
				<td><?php echo ($vo["phone"]); ?></td>
				<td><?php echo ($vo["email"]); ?></td>
				<td class="text-l"><?php echo ($vo['businessman'] == 1?'商户':'普通用户'); ?></td>
				<td><?php echo ($vo["time"]); ?></td>
				<td class="td-status"><span class="label label-success radius"><?php echo ($vo['status'] == 1?'正常':'禁用'); ?></span></td>
				<td class="td-manage" >
<!-- <?php if($v["status"] == 1 ): ?>-->
					<!-- <a style="text-decoration:none" onClick="member_stop(this,'<?php echo ($vo["id"]); ?>')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>  -->
<!-- <?php else: ?> -->
					
					<a style="text-decoration:none" onClick="member_start(this,<?php echo ($vo["id"]); ?>)" href="javascript:;" title="正常"><i class="Hui-iconfont">&#xe631;</i></a>
<!--<?php endif; ?> -->
					<a title="添加" href="javascript:;" onclick="member_edit('添加','member-add.html','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>

					 <a style="text-decoration:none" class="ml-5" onClick="change_password('修改密码','<?php echo U('User/userPassword');?>?id=<?php echo ($vo["id"]); ?>','<?php echo ($vo["id"]); ?>','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont">&#xe63f;</i></a>
...
/*用户-批量删除*/
	...
	function member_start(obj,id){
		...
		// console.log(id);
		$.post('<?php echo U("User/start");?>',{id:id},function (data){

			if(data ==2){

				//把启动改为禁用
				// console.log(obj.span.val());	
				$(obj).parent().prev().children('span').html('正常');

			}else{
				//把禁用改为启动
				$(obj).parent().prev().children('span').html('禁用');
			}
