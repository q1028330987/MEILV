...
<div class="page-container">
	<form action="<?php echo U('Manager/adminList');?>" method="get">
		<input type="text" class="input-text" style="width:250px" placeholder="输入管理员名称" id="adminName" name="adminName">
		<button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>	
	</form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="<?php echo U(addManager);?>"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a></span> <span class="r">共有数据：<strong><?php echo ($total); ?></strong> 条</span> </div>
...
<?php if(is_array($list)): foreach($list as $key=>$v): ?><tbody>
			<tr class="text-c" data="<?php echo ($v["uid"]); ?>">
				...
				<td class="td-manage" data-id="<?php echo ($_SESSION['admin']['id']); ?>">
...
<?php else: ?>
				...
				<a title="编辑" href="/first/index.php/MeilvAdmin/Manager/saveManager?id=<?php echo ($v["uid"]); ?>"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_del(this,'<?php echo ($v["uid"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
...
function admin_del(obj,id){
	...
	if ($(obj).parent().attr('data-id') == id) {

		layer.msg('自己不能删除自己!',{icon:1,time:1000});
			...
			$.ajax({
				...
				success: function(data){
					if (data['status']!=0) {
						...
						layer.msg('你没有权限啊!',{icon:1,time:1000});
...
function admin_stop(obj,id){
	...
	var session = '<%=((User)Session["user"]).UserId%>';
	console.log(session);

	if ($(obj).parent().attr('data-id') == id) {

		layer.msg('自己不能停用自己!',{icon:1,time:1000});
		...
		layer.confirm('确认要停用吗？',function(index){
			...
			$.post("<?php echo U('Manager/status');?>", { "id":id },function(data){
