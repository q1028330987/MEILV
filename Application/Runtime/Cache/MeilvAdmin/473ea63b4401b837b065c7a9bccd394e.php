...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 餐厅管理 <span class="c-gray en">&gt;</span> 餐厅管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-sort" data-count="<?php echo ($count); ?>">
			<thead>
				<tr class="text-c">
			...
			<tbody>
				...
				<?php if(is_array($storeList)): foreach($storeList as $key=>$vo): ?><tr class="text-c">
						...
						<td><img width="100" src="/first/Public/<?php echo ($vo["pic"]); ?>"></td>
						<td><?php echo ($vo["name"]); ?></td>
						<td>
						<?php if($vo["status"] == 3): ?><span style="color:red"><?php echo ($vo["statusname"]); ?></span>
						<?php else: ?>
							<?php echo ($vo["statusname"]); endif; ?>
						</td>
						<td class="f-14 product-brand-manage">
						<?php if($vo["status"] != 3): ?><a href="javascript:;" class="feng" onclick="refuse(this)" store-id="<?php echo ($vo["id"]); ?>" uid="<?php echo ($vo["uid"]); ?>" status="<?php echo ($vo["status"]); ?>" style="color:red">拒绝</a><?php endif; ?>
							
						<a href="javascript:;" class="feng" onclick="permit(this)" store-id="<?php echo ($vo["id"]); ?>" uid="<?php echo ($vo["uid"]); ?>" status="<?php echo ($vo["status"]); ?>" style="color:lightgreen">允许</a>
						<a href="<?php echo U('Store/goods_index');?>?id=<?php echo ($vo["id"]); ?>">详情</a></td>
...
// 	$.post('<?php echo U("Store/application");?>',
// 		{
	...
	$.post('<?php echo U("Store/application");?>',
		...
		function (data) {
			...
			if (data == 1) {
				...
				that.parent().prev().html('<span style="color:red">已拒绝</span>');
				// that.prev().remove();
				that.remove();
