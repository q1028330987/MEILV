...
<!--[if (gt IE 9)|!(IE)]><!--><html><!--<![endif]-->
  <head>
    ...
    <?php if(empty($_SESSION['userinfo'])): ?><span id="dis-login" class="top-disloginbar fl">
        <a class="j-register register-btn fl" style="color:#333" href="<?php echo U('Register/userRegister');?>" rel="nofollow">注册</a>
        <span class="lg-divide fl">|</span>
        <a class="j-login login-btn fl" style="color:#333" href="<?php echo U('Login/LoginExtends');?>" rel="nofollow">登录</a>
    </span>
    ...
    <?php else: ?>
      ...
      <span class="fl top-loginbar-username">欢迎你，<?php echo ($_SESSION['userinfo']['name']); ?></span>
        ...
        <li><a class="wrap" href="<?php echo U('Out/outLogin');?>" id="logout">退出</a></li>
              ...
              <a target="_blank" href="<?php echo U('Index/index');?>" class="site-name fl" ><i class="icon i-top-tuan"></i><span>美旅网</span></a>
              <a target="_blank" href="javascript:;" class="site-name fl" ><i class="icon i-top-call"></i><span>联系我们</span></a>
            </div>
            ...
            <i class="fl icon i-top-loc"></i>
              ...
              <span class="address fl" id="address">
                ...
                <div class="change fl clearfix" id="changePosition">
                  <a href="javascript:;" class="change-link">
...
<div class="account-cont clearfix" style="border:none;background:white">

      
	<div class="rest-banner">
	      <div selected-id="<?php echo ($selected); ?>"  class="imgsort-wrapper">
		        <span class="imgsort-filter-title">商家分类</span>
		        <ul class="clearfix imgsort-content">
		          <li class="fl typeAll selected">
		            <a href="<?php echo U('Index/index');?>" data-cate="cate_all" class="imgsort-list" title="全部">
		           ...
		           <?php if(is_array($typeList)): foreach($typeList as $key=>$vo): ?><li class="fl type" data-id="<?php echo ($vo["id"]); ?>" selected-id="<?php echo ($selected); ?>">
			              <a class="imgsort-list" title="<?php echo ($vo["name"]); ?>"  href="<?php echo U('Index/index', ['id'=>$vo['id']]);?>" data-cate="" >
			                <span class="imgsort-info"><?php echo ($vo["name"]); ?></span>
			              </a>
		...
		<?php if(is_array($storeList)): foreach($storeList as $key=>$vo): ?><li class="fl rest-li">
      				<div  class="j-rest-outer self rest-outer transition">     				
					      <div data-title="<?php echo ($vo["name"]); ?>" data-bulletin="" data-poiid="144871167931143723" class="restaurant" data-all="1" data-invoice="1" data-minpricelevel="1">
					        <a class="rest-atag" href="<?php echo U('Store/store', ['id'=>$vo['id'], 'level'=>$vo['level'], 'levelper'=>$vo['levelper']]);?>" target="_blank">
					          <div class="top-content">
					            <div class="preview">
					              <img data-rid="144871167931143723" data-index="0" class="scroll-loading" src="/first/Public/<?php echo ($vo["pic"]); ?>" data-src="/first/Public/<?php echo ($vo["pic"]); ?>" data-max-width="208" data-max-height="156">
					              <div class="rest-tags">
					              </div>
					            </div>
					            <div class="content">
					              <div class="name">
					                <span title="<?php echo ($vo["name"]); ?>">
					   		<?php echo ($vo["name"]); ?>
					                ...
					                <div class="rank clearfix">
					                  <span class="star-ranking fl">
					                    <!-- 5颗星60px长度，算此时星级的长度 -->
					                    <!-- 算出空白填充的部分长度 -->
					                    <span class="star-score" style="width: <?php echo ($vo["levelper"]); ?>px"></span>
					                  </span>

					                  <?php if($vo["level"] == 0): ?><span class="score-num fl">暂无</span>
							
					                  <?php else: ?>

					                  <span class="score-num fl"><?php echo ($vo["level"]); ?>分</span><?php endif; ?>
					                  <!--
					...
					月售3322单
					              ...
					              <div class="price">
					                <span class="start-price">起送:￥<?php echo ($vo["upsend"]); ?></span>
					                <span class="send-price">
					                  配送费:￥<?php echo ($vo["peisend"]); ?>
					                </span>
					                <span class="send-time"><i class="icon i-poi-timer"></i>
					                      <?php echo ($vo["atime"]); ?>分钟
					                </span>
					              </div>
					            </div>
					            <div class="clear"></div>
					        	...
					        	<?php if($vo["collect"] == 1): ?><a href="javascript:;" data-id='<?php echo ($vo["id"]); ?>' class="un-favorite j-save-up" data-status="1" title="收藏商家">
						           		<i class="icon i-poi-fav1" style="background-position:-368px -312px" ></i>
						          	</a>

					        	<?php else: ?>
							<a href="javascript:;" class="un-favorite j-save-up" data-status="0" data-id="<?php echo ($vo["id"]); ?>" title="收藏商家">
		...
		<li class="rest-separate j-rest-separate loading" id="loading">
      	...
      	<script>
      		...
      		$('.un-favorite').on('click', function () {
      			var userid = '<?php echo ($_SESSION['userinfo']['id']); ?>';
      			if (userid !== '') {
		...
		function isEmptyObject(obj) {
			...
			return true;
      		...
      		$('#loading').on('click', function () {
      			...
      			$.post('<?php echo U("Index/ajaxIndex");?>',
      				{limit : limit,
      				...
      				function (data) {
						...
						$('.isloading').html('扫二维码查看更多商家');
      						...
      						var id = data[i].id;
      						var href = "<?php echo U('Home/Store/store/id/"+id+"');?>";
      						
      						console.log(href);

      						str += '<li class="fl rest-li"><div  class="j-rest-outer self rest-outer transition">     				<div data-title="'+data[i].name+'" data-bulletin="" data-poiid="144871167931143723" class="restaurant" data-all="1" data-invoice="1" data-minpricelevel="1"><a class="rest-atag" href="'+href+'" target="_blank"><div class="top-content"><div class="preview"><img data-rid="144871167931143723" data-index="0" class="scroll-loading" src="/first/Public/'+data[i].pic+'" data-src="/first/Public/'+data[i].pic+'" data-max-width="208" data-max-height="156"><div class="rest-tags"></div></div><div class="content"><div class="name"><span title="'+data[i].name+'">'+data[i].name+'</span></div><div class="rank clearfix"><span class="star-ranking fl"><span class="star-score" style="width: '+data[i].levelper+'px"></span> </span>';
    ...
    <div class="page-footer">
      <div class="footer-wrap">
        ...
        <div class="column fl corp">
          ...
          <ul>
            <li><a href="" class="ca-darkgrey kaidian_address" target="_blank">我要开店</a></li>
            <li><a href="" class="ca-darkgrey" target="_blank">配送合作申请入口</a></li>
            <li><a href="" class="ca-darkgrey" target="_blank">城市代理商申请入口</a></li>
            <li><a href="" class="ca-darkgrey" target="_blank">零售招商合作加盟入口</a></li>
            <li><a href="" class="ca-darkgrey" target="_blank">聚宝盆餐饮开放平台</a></li>
          </ul>
        </div>
        <div class="column fl info">
          <div class="title">友情链接</div>
          <?php if(is_array($list)): foreach($list as $key=>$vo): ?><ul>
            <li><a href="<?php echo ($vo["url"]); ?>" class="ca-darkgrey" target="_blank" rel="nofollow"><?php echo ($vo["name"]); ?></a></li>
        ...
        <div class="clear"></div>
        <div class="copyright">&copy;2015 meituan.com <a target="_blank" href="javascript:;">京ICP证070791号</a> 京公网安备11010502025545号</div>
        <div class="sp-ft">
          <a target="_blank" title="备案信息" href="" class="record"></a>
        </div>
      </div>
    </div>
  </div>
...
<script src="/first/Public/js/rohr.min.js"></script>
