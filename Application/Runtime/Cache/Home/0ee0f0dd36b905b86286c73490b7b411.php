...
<div class="account-cont clearfix">
  ...
  <div class="orders-tab fl">
    ...
    <div class="user-info clearfix ct-commonblack">
       <form action="<?php echo U('ManStore/saveGoods');?>" enctype="multipart/form-data" method="post">
        <div class="form-group">
          <label for="exampleInputEmail1">商品分类</label>
        <select name="tid" class="form-control" style="width: 160px">
<?php if(is_array($list)): foreach($list as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php echo ($v['id']==$goods['tid']?selected:''); ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
        </select>

        </div>
        <div class="form-group">
          <label for="name">商品名</label>
          <input type="text" class="form-control" value="<?php echo ($goods["name"]); ?>" style="width: 250px;" name="name" id="name" placeholder="请输入店铺名">
          <input type="hidden" name="id" value="<?php echo ($goods["id"]); ?>">
        </div>
        <div class="form-group">
          <label for="price">商品价格</label>
          <input type="text" class="form-control" value="<?php echo ($goods["price"]); ?>" style="width: 250px;" name="price" id="price" placeholder="请输入商品价格">
        </div>
        <div class="form-group">
          <label for="des">商品描述</label>

          <input type="text" class="form-control" value="<?php echo ($goods["des"]); ?>"  name="des" id="des" placeholder="请输入商品描述">
        </div>
        <div class="form-group">
          <label for="exampleInputFile">原商品照片</label>
          <img src="/first/Public/<?php echo ($goods["pic"]); ?>" height="50px" alt="">
          <input type="file" name='pic' id="exampleInputFile">
          <p class="help-block">不添加照片等于不修改商品相片</p>
        </div>
